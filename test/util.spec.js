...
describe('mongo-query utility belt test', function () {
	...
	describe('when perfoming update operation', function () {
		...
		describe('#testing array modifiers', function () {
			...
			describe('#testing string based deep modifiers', function () {
			...
			describe('#testing string based deep modifiers when array', function () {
				_(db.beverages).update({name: 'ZeroVatkaCola'}, {$set: {'someArray.0': 'FIRSTITEM'}});

				it('#should properly set up first item', function () {
					var doc = _(db.beverages).findOne({name: 'ZeroVatkaCola'});
					expect(doc.someArray[0]).to.equal('FIRSTITEM');
				});
			});
			...
			describe('#testing string based deep modifiers when array with deep nesting', function () {
				...
				it('#should properly set up first item', function () {
					var doc = _(db.beverages).findOne({name: 'ZeroVatkaCola'});
					expect(doc.nestedObjectsArray[0].someProperty).to.equal('value modified');
				});
			});
		});
	});
